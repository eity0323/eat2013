	...
	public void onEventMainThread(AsyncResult bean){
		switch(bean.getState()){
			case REQUEST_SUCCESS_CODE:
				if(!bean.getListener().interceptBeforeSuccess(bean.getRequestCode(), bean.getResult())){
			...
			case HTTP_NULL_CODE:
				if(!bean.getListener().interceptBeforeFailure(bean.getRequestCode(), bean.getState(), bean.getResult())){
